<template>
  ...
  >
    <template v-slot:left>
      ...
      <analytics-list>
        <analytics-list-item
          v-if="nft.meta.externalUrl"
          :is-loading="isStoreLoading"
          :title="$t('NFTs.lblViewOnUns')"
        >
          <template>
            <a
              class="link emphasize bold"
              :href="nft.meta.externalUrl"
              rel="external help nofollow"
              target="_blank"
            >
              {{ nft.meta.externalUrl }}
            </a>
          </template>
        </analytics-list-item>
      </analytics-list>
    ...
    <template v-slot:right>
      <custom-picture :src="nft.picture.src" :style="pictureStyle" />
    </template>
  </content-wrapper-two-sided>
</template>
...
export default Vue.extend({
  ...
  computed: {
    ...
    pictureStyle(): CSSProperties {
      if (this.nft.meta.backgroundColor !== undefined) {
        return {
          backgroundColor: `#${this.nft.meta.backgroundColor}`
        };
      }
