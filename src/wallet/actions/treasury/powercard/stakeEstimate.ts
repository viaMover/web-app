...
export const estimateStakePowercardCompound = async (
  network: Network,
  web3: Web3,
  ...
  if (!isApproved) {
    ...
    const depositEstimate = await estimatePowercardStake(
      accountAddress,
      contractAddress,
      web3
    );
...
export const estimatePowercardStake = async (
  accountAddress: string,
  contractAddress: string,
  web3: Web3
): Promise<EstimateResponse> => {
  console.log('Estimating powercard stake...');
  ...
  try {
    ...
    if (gasLimitObj) {
      ...
      console.log(
        '[powercard stake estimation] gas estimation by web3: ' + gasLimit
      );
      const gasLimitWithBuffer = floorDivide(multiply(gasLimit, '120'), '100');
      console.log(
        '[powercard stake estimation] gas estimation by web3 (with additional 20% as buffer): ' +
          gasLimitWithBuffer
      );
    ...
    console.error(`can't estimate powercard stake due to: ${error}`);
    return {
      error: true,
      gasLimit: '0'
    };
